module TestAssetThermalMethanolBalance
...
function make_thermalmethanol_case(style::Symbol)
    system = make_test_system([Methanol, Electricity, NaturalGas, CO2])
    ...
    push_locations!(system, fuel_source, elec_source, ch3oh_sink, co2_sink)

    asset = make(
        ThermalMethanol,
        Dict{Symbol,Any}(
            :id => :thermal_methanol_test,
            :timedata => "Methanol",
    ...
    transform = asset.thermalmethanol_transform
    ...
    elseif style != :default
        error("Unsupported ThermalMethanol balance style: $style")
    end

    return (; system, asset)
end

function assert_thermalmethanol_solution(asset, model)
...
function test_asset_thermalmethanol_balance()
    @testset "ThermalMethanol Small Solve Cases" begin
        default_case = make_thermalmethanol_case(:default)
        add_balance_case = make_thermalmethanol_case(:add_balance)

        default_model = build_test_model(default_case.system)
        add_balance_model = build_test_model(add_balance_case.system)

        assert_thermalmethanol_solution(default_case.asset, default_model)
        assert_thermalmethanol_solution(add_balance_case.asset, add_balance_model)
...
test_asset_thermalmethanol_balance()

end # module TestAssetThermalMethanolBalance
