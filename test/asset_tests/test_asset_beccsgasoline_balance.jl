module TestAssetBECCSGasolineBalance
...
function make_beccsgasoline_case(style::Symbol)
    ...
    push_locations!(system, biomass_source, elec_source, co2_source, gasoline_sink, co2_emission_sink, co2_captured_sink)

    asset = make(
        BECCSGasoline,
        Dict{Symbol,Any}(
            :id => :beccs_gasoline_test,
            :timedata => "Biomass",
            :can_expand => false,
            :can_retire => false,
            :existing_capacity => 100.0,
            :gasoline_production => 0.5,
    ...
    if style == :add_balance
        @add_balance(transform, :gasoline, 0.5 * flow(asset.biomass_edge) == flow(asset.gasoline_edge))
    ...
    elseif style == :stoich
        @add_stoichiometric_balance(
            ...
            -->
            0.5 * flow(asset.gasoline_edge) + 0.15 * flow(asset.co2_emission_edge) + 0.25 * flow(asset.co2_captured_edge),
            flow(asset.biomass_edge),
        )
    else
        error("Unsupported BECCSGasoline balance style: $style")
    end

    return (; system, asset)
end

function assert_beccsgasoline_solution(asset, model)
...
function test_asset_beccsgasoline_balance()
    @testset "BECCSGasoline Small Solve Cases" begin
        add_balance_case = make_beccsgasoline_case(:add_balance)
        stoich_case = make_beccsgasoline_case(:stoich)

        add_balance_model = build_test_model(add_balance_case.system)
        stoich_model = build_test_model(stoich_case.system)

        assert_beccsgasoline_solution(add_balance_case.asset, add_balance_model)
        assert_beccsgasoline_solution(stoich_case.asset, stoich_model)
...
test_asset_beccsgasoline_balance()

end # module TestAssetBECCSGasolineBalance
