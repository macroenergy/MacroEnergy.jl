...
function test_min_capacity()
    @testset "MinCapacityConstraint (system/location)" begin
        @testset "system-wide scope" begin
            system = build_system()
            ct = MinCapacityConstraint(; config = vre_cfg(1.0))
        ...
        @testset "per-location scope" begin
            system = build_system()
            ctA = MinCapacityConstraint(; config = vre_cfg(1.0))
            ctB = MinCapacityConstraint(; config = vre_cfg(1.0))
            # Location :C has a VRE floor configured but no VRE assets located there.
            ctC = MinCapacityConstraint(; config = vre_cfg(1.0))
        ...
        @testset "parameter scaling of RHS" begin
            system = build_system()
            ctsys = MinCapacityConstraint(; config = vre_cfg(1000.0))
            ctloc = MinCapacityConstraint(; config = vre_cfg(300.0))
