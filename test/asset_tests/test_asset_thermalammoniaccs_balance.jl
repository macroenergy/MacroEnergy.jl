module TestAssetThermalAmmoniaCCSBalance
...
function make_thermalammoniaccs_case(style::Symbol)
    system = make_test_system([Ammonia, Electricity, NaturalGas, CO2, CO2Captured])
    ...
    push_locations!(system, fuel_source, elec_source, nh3_sink, co2_sink, co2_captured_sink)

    asset = make(
        ThermalAmmoniaCCS,
        Dict{Symbol,Any}(
            :id => :thermal_ammonia_ccs_test,
            :timedata => "Ammonia",
    ...
    transform = asset.thermalammoniaccs_transform
    if style == :add_balance
        ...
        @add_balance(transform, :capture, 0.3 * flow(asset.nh3_edge) == flow(asset.co2_captured_edge))
    elseif style != :default
        error("Unsupported ThermalAmmoniaCCS balance style: $style")
    end

    return (; system, asset)
end

function assert_thermalammoniaccs_solution(asset, model)
...
function test_asset_thermalammoniaccs_balance()
    @testset "ThermalAmmoniaCCS Small Solve Cases" begin
        default_case = make_thermalammoniaccs_case(:default)
        add_balance_case = make_thermalammoniaccs_case(:add_balance)

        default_model = build_test_model(default_case.system)
        add_balance_model = build_test_model(add_balance_case.system)

        assert_thermalammoniaccs_solution(default_case.asset, default_model)
        assert_thermalammoniaccs_solution(add_balance_case.asset, add_balance_model)
...
test_asset_thermalammoniaccs_balance()

end # module TestAssetThermalAmmoniaCCSBalance
