...
function add_model_constraint!(ct::DeploymentInertiaConstraint, system::System, model::Model, settings::NamedTuple)
    ...
    if p_idx == 1
        # CAGR
        ct.constraint_ref = @constraint(model, [t in settings[:TechsWithInertia]], model[:eDeploymentGrowth][t, p_idx] <= settings[:InertiaInit][Symbol(t)] * (1 + settings[:InertiaInitialGrowthRate][Symbol(t)]))
        # CADR
        ct.constraint_ref = @constraint(model, [t in settings[:TechsWithInertia]], model[:eDeploymentDecline][t, p_idx] >= settings[:InertiaInit][Symbol(t)] * (1 - settings[:InertiaInitialDeclineRate][Symbol(t)]))
    elseif p_idx > 1
        # CAGR
